...
class ServiceController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            //
            /*$sqlQuery = "select `motor_custome_values`.make_id, `make_msts`.`name` as `makename` from `motor_custome_values` left join `make_msts` on `motor_custome_values`.`make_id` = `make_msts`.`id` left join `ads` on `motor_custome_values`.`ads_id` = `ads`.`id` where make_msts.status=1 group by make_id,makename order by sort_order";*/
            $sqlQuery = "select `motor_custome_values`.make_id, `make_msts`.`name` as `makename`,delete_status from `motor_custome_values` left join `make_msts` on `motor_custome_values`.`make_id` = `make_msts`.`id` left join `ads` on `motor_custome_values`.`ads_id` = `ads`.`id` where make_msts.status=1 AND delete_status=0 group by make_id,makename,delete_status order by make_msts.sort_order";
...
$sqlQuery = "select `motor_custome_values`.model_id, `model_msts`.`name` as `modelname`,delete_status from `motor_custome_values` left join `model_msts` on `motor_custome_values`.`model_id` = `model_msts`.`id` left join `ads` on `motor_custome_values`.`ads_id` = `ads`.`id`  where model_msts.status=1 AND delete_status=0 group by model_id,modelname,delete_status order by model_msts.sort_order";
            ...
            $sqlQuery = "select distinct registration_year,DELETE_status from motor_custome_values left join `ads` on `motor_custome_values`.`ads_id` = `ads`.`id` WHERE delete_status=0 order by registration_year";
            ...
            if ($pageflag == 1) {
               ...
               /* $sqlQuery = "select `motor_custome_values`.make_id, `make_msts`.`name` as `makename` from `motor_custome_values` left join `make_msts` on `motor_custome_values`.`make_id` = `make_msts`.`id` group by make_id,makename";
                ...
                if (($year != '0') && ($make == '0') && ($model == '0') && ($priceflag == 0) && ($keywordsearch == '')) {
                    ...
                    $vehicletypecars = Ads::select("ads.*", "ads.canonical_name as mainid", "vehicletype.*",  'motor_custome_values.*',"ads.id as mainid1",  "model_msts.name as modelname", 'places.name as placename', 'countries.name as countryname', "make_msts.name as makename")->leftjoin("motor_custome_values", "ads.id", "=", "motor_custome_values.ads_id")->leftjoin("vehicletype", "ads.vehicletype", "=", "vehicletype.id")->leftjoin("model_msts", "motor_custome_values.model_id", "=", "model_msts.id")->leftjoin("places", "places.id", "=", "ads.place")->leftjoin("countries", "countries.id", "=", "ads.country_id")->leftjoin("make_msts", "motor_custome_values.make_id", "=", "make_msts.id")->where("ads.delete_status", 0)->where("motor_custome_values.registration_year", $year)->orderBy('ads.sortorder')->paginate(9);
                } else if (($year != '0') && ($make == '0') && ($model == '0') && ($priceflag != 0) && ($keywordsearch == '')) {
                    // "year and price";
                    $flag=10;
                    $vehicletypecars = Ads::select("ads.*", "ads.canonical_name as mainid", "vehicletype.*", 'motor_custome_values.*',"ads.id as mainid1",  'places.name as placename', 'countries.name as countryname', "model_msts.name as modelname", "make_msts.name as makename")->leftjoin("motor_custome_values", "ads.id", "=", "motor_custome_values.ads_id")->leftjoin("vehicletype", "ads.vehicletype", "=", "vehicletype.id")->leftjoin("places", "places.id", "=", "ads.place")->leftjoin("countries", "countries.id", "=", "ads.country_id")->leftjoin("model_msts", "motor_custome_values.model_id", "=", "model_msts.id")->leftjoin("make_msts", "motor_custome_values.make_id", "=", "make_msts.id")->where("ads.delete_status", 0)->where("motor_custome_values.registration_year", $year)->where('ads.price', '>=', "$minpriceval")->where('ads.price', '<=', "$maxpriceval")->orderBy('ads.sortorder')->paginate(9);
                } else if (($year == '0') && ($make != '0') && ($model == '0') && ($priceflag == 0) && ($keywordsearch == '')) {
                    // "make alone";
                    $flag=11;
...
//dd($quries);
    ...
    public function getModel($makeid = 0)
    {
        ...
        $modelData['data'] = MotorCustomeValues::leftjoin("model_msts", "motor_custome_values.model_id", "=", "model_msts.id")
            ->select('model_msts.id', 'name')->leftjoin("ads","ads.id","=","motor_custome_values.ads_id")->groupby('model_msts.id', 'name')
            ->where('motor_custome_values.make_id', $makeid)->where("model_msts.status",1)->where('delete_status',0)->orderby('model_msts.sort_order')
            ->get();
